...
from cnn_detections.gridmodel import GridModel
from cnn_detections.ball_tracker import BallTracker
    ...
    for frame_count in range(total_frames):
        ...
        if len(tracked_balls) == n_balls:
            for i in range(n_balls):
                tracking_data["balls"][i].append(tuple(tracked_balls[i]))
...
def analyze_ball_states(tracking_data, hand_zone_dims=(120, 60)):
    ...
    for frame_idx in range(n_frames):
        left_pos, right_pos = np.array(
            tracking_data["left_hand_pos"][frame_idx]
        ), np.array(tracking_data["right_hand_pos"][frame_idx])
        for ball_idx in range(n_balls):
            ball_pos = np.array(tracking_data["balls"][ball_idx][frame_idx])
            is_in_left = (abs(ball_pos[0] - left_pos[0]) < half_width) and (
                abs(ball_pos[1] - left_pos[1]) < half_height
            )
            is_in_right = (abs(ball_pos[0] - right_pos[0]) < half_width) and (
                abs(ball_pos[1] - right_pos[1]) < half_height
            )
            state = "in_flight"
            if is_in_left and is_in_right:
                state = (
                    "left_hand"
                    if np.linalg.norm(ball_pos - left_pos)
                    <= np.linalg.norm(ball_pos - right_pos)
                    else "right_hand"
                )
            elif is_in_left:
                state = "left_hand"
