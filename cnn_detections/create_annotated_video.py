...
from drawingutils import draw_stateful_annotations
...
def create_annotated_video(video_path, output_path, tracking_data, ball_states):
    ...
    for frame_idx in range(total_frames):
        ...
        current_ball_positions = [tracking_data['balls'][b][frame_idx] for b in range(len(ball_states))]
        current_ball_states = [ball_states[b][frame_idx] for b in range(len(ball_states))]

        # Draw the stateful annotations on the frame
        draw_stateful_annotations(cropped_frame, lhand_pos, rhand_pos, current_ball_positions, current_ball_states)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Create a state-annotated video of a juggling performance.")
    ...
    parser.add_argument('--output', type=str, default='output_visuals.mp4', help="Path to save the final annotated video.")
    parser.add_argument('--n_balls', type=int, default=3, help="Number of balls to detect.")
